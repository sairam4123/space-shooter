...
signal joystick_updated(position)
...
var InputEventTouchMouseButton
var InputEventTouchMouseMotion

func _ready() -> void:
	if not OS.has_touchscreen_ui_hint() and visibility_mode == VisibilityMode.TOUCHSCREEN_ONLY:
		hide()
	InputEventTouchMouseButton = InputEventScreenTouch
	InputEventTouchMouseMotion = InputEventScreenDrag

func _touch_started(event) -> bool:
	return event.pressed and _touch_index == -1

func _touch_ended(event) -> bool:
	return not event.pressed and _touch_index == event.button_index if InputEventTouchMouseButton == InputEventMouseButton else event.index

func _input(event: InputEvent) -> void:
	if not (event is InputEventTouchMouseButton or event is InputEventTouchMouseMotion):
		return

	if event is InputEventTouchMouseButton:
		if _touch_started(event) and _is_inside_control_rect(event.position, self):
			...
			if _is_inside_control_circle(event.position, _background):
				_touch_index = event.button_index if InputEventTouchMouseButton == InputEventMouseButton else event.index
	...
	elif event is InputEventTouchMouseMotion and _touch_index == event.button_index if InputEventTouchMouseMotion == InputEventMouseMotion else event.index:
		_update_joystick(event.position)
...
func _update_joystick(event_position: Vector2):
	...
	if vector.length() > dead_size:
		...
		is_working = false
		output = Vector2.ZERO
		emit_signal("joystick_updated", Vector2.ZERO)
		_reset_handle()
